...
public final class StaticTableSchema<T> implements TableSchema<T> {
    private final ImmutableTableSchema<T, T> delegateImmutableTableSchema;

    private StaticTableSchema(Builder<T> builder) {
        this.delegateImmutableTableSchema = builder.delegateBuilder.build();
    }
    ...
    public static final class Builder<T> {
        private final ImmutableTableSchema.Builder<T, T> delegateBuilder;
        private final Class<T> itemClass;

        private Builder(Class<T> itemClass) {
            this.delegateBuilder = ImmutableTableSchema.builder(itemClass, itemClass);
    ...
    public StaticTableMetadata tableMetadata() {
        return this.delegateImmutableTableSchema.tableMetadata();
    }

    @Override
    public T mapToItem(Map<String, AttributeValue> attributeMap) {
        return this.delegateImmutableTableSchema.mapToItem(attributeMap);
    }

    @Override
    public Map<String, AttributeValue> itemToMap(T item, boolean ignoreNulls) {
        return this.delegateImmutableTableSchema.itemToMap(item, ignoreNulls);
    }

    @Override
    public Map<String, AttributeValue> itemToMap(T item, Collection<String> attributes) {
        return this.delegateImmutableTableSchema.itemToMap(item, attributes);
    }

    @Override
    public AttributeValue attributeValue(T item, String key) {
        return this.delegateImmutableTableSchema.attributeValue(item, key);
    }

    @Override
    public EnhancedType<T> itemType() {
        return this.delegateImmutableTableSchema.itemType();
    }

    @Override
    public List<String> attributeNames() {
        return this.delegateImmutableTableSchema.attributeNames();
    }

    @Override
    public boolean isAbstract() {
        return this.delegateImmutableTableSchema.isAbstract();
    }
    ...
    public AttributeConverterProvider attributeConverterProvider() {
        return this.delegateImmutableTableSchema.attributeConverterProvider();
    }

    ImmutableTableSchema<T, T> toImmutableTableSchema() {
        return this.delegateImmutableTableSchema;
    }
}
